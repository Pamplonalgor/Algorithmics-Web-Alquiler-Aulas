    ...
    return (
      <main className="pt-32 min-h-screen flex items-center justify-center bg-slate-950 px-4">
        <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 max-w-md w-full text-center shadow-2xl">
          <h1 className="text-3xl font-bold mb-8 text-amber-500">Acceso Admin</h1>
          <form onSubmit={handleLogin} className="space-y-6">
            <input type="password" placeholder="Contraseña" className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl text-white outline-none focus:border-amber-500 transition-colors" value={password} onChange={(e) => setPassword(e.target.value)} required />
            <button type="submit" className="w-full bg-amber-500 text-slate-950 font-bold py-4 rounded-xl hover:bg-amber-400 transition-all">Entrar al Panel</button>
          </form>
        </div>
      </main>
    );
    ...
    <main className="pt-32 min-h-screen bg-slate-950 text-white pb-20">
      <section className="max-w-6xl mx-auto px-4 py-12">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <h1 className="text-4xl font-bold tracking-tighter">Panel de <span className="text-amber-500">Administración</span></h1>
          ...
          {['servicios', 'formacion', 'aulas', 'reservas', 'config'].map((tab) => (
            <button
              ...
              className={`px-8 py-3 rounded-full font-bold capitalize transition-all ${
                activeTab === tab ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20' : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
          ...
          {activeTab === 'config' ? (
            <div className="max-w-xl mx-auto text-center py-12">
              ...
              <div className="space-y-6 text-left">
                <div className="flex flex-col gap-3">
                  ...
                  <input type="email" value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl focus:border-amber-500 outline-none text-white font-bold" />
                </div>
                <button onClick={handleSaveEmail} disabled={isSavingEmail} className="w-full bg-amber-500 text-slate-950 font-black py-5 rounded-2xl hover:bg-amber-400 transition-all shadow-xl shadow-amber-500/20 disabled:opacity-50">
            ...
            <>
              <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6">
                ...
                {activeTab !== 'reservas' && (
                  <button onClick={() => { setEditingItem(null); setShowForm(true); }} className="bg-amber-500 hover:bg-amber-400 text-slate-950 px-8 py-3 rounded-full font-bold transition-all flex items-center gap-2">
                        ...
                        <tr key={item.id} className="group hover:bg-white/5 transition-colors">
                          ...
                          {activeTab === 'reservas' ? (
                            <>
                              ...
                              <td className="py-6 px-4"><div className="text-amber-500 font-bold">{item.total_price}€</div></td>
                            </>
                          ) : (
                            <td className="py-6 px-4 font-semibold">{item.icon && <span className="mr-3 text-xl">{item.icon}</span>}{item.title || item.name}</td>
                          )}
